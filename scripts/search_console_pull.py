...
OUT=ROOT/"docs/growth/search_console_latest.json"
...
def main():
    ...
    try:
        ...
        body={"startDate":start.isoformat(),"endDate":end.isoformat(),"dimensions":["page","query"],"rowLimit":25000,"dataState":"final"}
    ...
    for r in rows:
        keys=r.get("keys",["",""])
        normalized.append({"page":keys[0] if len(keys)>0 else "","query":keys[1] if len(keys)>1 else "","clicks":r.get("clicks",0),"impressions":r.get("impressions",0),"ctr":r.get("ctr",0),"position":r.get("position",0)})
    roll={}
    for r in normalized:
        ...
        if len(cur["queries"])<20: cur["queries"].append({k:r[k] for k in ("query","clicks","impressions","ctr","position")})
    pages=[]
    for p,v in roll.items():
        imp=v["impressions"]
        pages.append({"page":p,"clicks":round(v["clicks"],2),"impressions":round(imp,2),"ctr":round(v["clicks"]/imp,4) if imp else 0,"position":round(v["weighted_position"]/imp,2) if imp else 0,"queries":sorted(v["queries"],key=lambda x:x["impressions"],reverse=True)})
    ...
    print("Wrote",OUT.relative_to(ROOT),"pages=",len(pages)); return 0
