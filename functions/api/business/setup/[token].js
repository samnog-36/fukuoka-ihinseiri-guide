...
export async function onRequestPost(context){
  ...
  if(existing){
    await db.prepare(`UPDATE advertisements SET
      ...
      service_genres_json=?,service_area=?,updated_at=? WHERE business_id=?`)
      .bind(
        ...
        clean(input.qualifications,1200)||null,JSON.stringify(genres),clean(input.serviceArea,1200)||b.service_area,now,b.id
      ).run();
  }else{
    await db.prepare(`INSERT INTO advertisements
      (id,business_id,company_name,catchphrase,description,phone,email,price_range,business_hours,qualifications,service_genres_json,service_area,is_active,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?, ?,0,?,?)`)
      .bind(
        rowId("ad"),b.id,b.company_name,clean(input.catchphrase,200)||null,clean(input.description,3000)||null,
        clean(input.phone,80)||b.phone,email,clean(input.priceRange,240)||null,clean(input.businessHours,240)||null,
        clean(input.qualifications,1200)||null,JSON.stringify(genres),clean(input.serviceArea,1200)||b.service_area,now,now
      ).run();
  }
